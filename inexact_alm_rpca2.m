function [A_hat E_hat iter] = inexact_alm_rpca2(D, lambda, eta, tol, maxIter)
...
while ~converged
    ...
    mu_pline = 2 * (eta+mu/2);
    %if choosvd(n, sv) == 1
    %    [U S V] = lansvd(mu*mu_pline^-1*(D - E_hat + (1/mu)*Y), sv, 'L');
    %else
    [U S V] = svd(mu*mu_pline^-1*(D - E_hat + (1/mu)*Y), 'econ');
    %end
    diagS = diag(S);
    svp = length(find(diagS > 1/mu_pline));
    if svp < sv
        sv = min(svp + 1, n);
    else
    ...
    A_hat = U(:, 1:svp) * diag(diagS(1:svp) - 1/mu_pline) * V(:, 1:svp)';
    ...
    min_value = sum(diag(S)) + lambda * norm(E_hat,1) + eta * norm(A_hat, 'fro')^2;
    %disp([ 'min_value ='  num2str(min_value)])
    %% stop Criterion
